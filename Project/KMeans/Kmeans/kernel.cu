...
__global__ void CalculateCenterSumsKernelConstant(LocationConstant* locations, LocationConstant* centers, int amount_of_locations, int* centers_are_same, int amount_of_threads) {
    //VERY IMPORTANT TO INITIALIZE SHARED MEMORY
    __shared__ float sharedData[3072];
    // Initialize shared memory
    int index = threadIdx.x;
...
bool AssignLocationAndCalculateSums(vector<LocationConstant>* locations, vector<LocationConstant>* centers) {
    ...
    CalculateCenterSumsKernelConstant << <centers->size(), 1024 >> > (GPULocations, GPUCenters, locations->size(), GPUBools, 1024);
    cudaDeviceSynchronize();
    cudaMemcpy(centers->data(), GPUCenters, centers->size() * sizeof(LocationConstant), cudaMemcpyDeviceToHost);
    cudaMemcpy(done.data(), GPUBools, centers->size() * sizeof(int), cudaMemcpyDeviceToHost);
    //Free memory
    cudaFree(GPULocations);
    cudaFree(GPUCenters);
...
vector<Location> GetCenterLocations(vector<Location>* locations, vector<int> indices, int amount_of_centers) {
    vector<Location> centers;
    for (int i = 0; i < amount_of_centers; i++) {
...
vector<LocationConstant> GetCenterLocationsConstant(vector<LocationConstant>* locations, vector<int> indices, int amount_of_centers) {
...
void implementation_per_iteration() {
    resetOutputFiles();
    ...
    for (int i = 0; i < 5; i++) {
        ...
        vector<int> indices = getCenterpointIndices(locations.size(), K / count);

        vector<Location> centers = GetCenterLocations(&locations, indices, indices.size());
        vector<Location> centers2 = GetCenterLocations(&locations2, indices, indices.size());
        vector<Location> centers3 = GetCenterLocations(&locations3, indices, indices.size());
        vector<LocationConstant> centers_constant = GetCenterLocationsConstant(&locations_constant, indices, indices.size());
        ...
        KmeansCPU(&locations, 10000, K / count, &centers);
        ...
        KmeansGPU(&locations2, 10000, K / count, &centers2);
        ...
        KmeansGPUConstant(&locations_constant, 10000, K / count, &centers_constant);
        ...
        KmeansGPUWithSum(&locations3, 10000, K / count, &centers3);
        ...
        cout << "Amount of clusters: " << K / count << endl;
        ...
        cout << elapsed_secondsCPU.count() << endl;
        cout << elapsed_secondsGPU.count() << endl;
        cout << elapsed_secondsGPUC.count()<< endl;
        cout << elapsed_secondsGPUS.count() << endl;
...
void implementation_per_implementation() {
    resetOutputFiles();
    vector<Location> locations = readLocationsCsv(); //Get locations
    //vector<Location> locations2 = readLocationsCsv();
    vector<LocationConstant> locations_constant = readLocationsConstantCsv();
    //vector<LocationConstant> locations_constant2 = readLocationsConstantCsv();
    //vector<Location> locations3 = readLocationsCsv();
    vector<int> indices = getCenterpointIndices(locations.size(), K);
    int amount_of_iterations = 5;

    vector<double> CPUtimes;
    int count = 1;
    for (int i = 0; i < amount_of_iterations; i++) {
        vector<Location> centers = GetCenterLocations(&locations, indices, K / count);

        const auto startCPU = std::chrono::steady_clock::now();
        KmeansCPU(&locations, 10000, K / count, &centers);
        const auto endCPU = std::chrono::steady_clock::now();
        ResetLocationClusters(&locations);
        CPUtimes.push_back(std::chrono::duration<double, milli>(endCPU - startCPU).count());
        count *= 2;
    }

    vector<double> GPUtimes;
    count = 1;
    for (int i = 0; i < amount_of_iterations; i++) {
        vector<Location> centers = GetCenterLocations(&locations, indices, K /count);
        const auto start = std::chrono::steady_clock::now();
        KmeansGPU(&locations, 10000, K / count, &centers);
        const auto end = std::chrono::steady_clock::now();
        ResetLocationClusters(&locations);
        GPUtimes.push_back(std::chrono::duration<double, milli>(end - start).count());
        count *= 2;
    }

    vector<double> GPUSumtimes;
    count = 1;
    for (int i = 0; i < amount_of_iterations; i++) {
        vector<Location> centers = GetCenterLocations(&locations, indices, K / count);
        const auto start = std::chrono::steady_clock::now();
        KmeansGPUWithSum(&locations, 10000, K / count, &centers);
        const auto end = std::chrono::steady_clock::now();
        ResetLocationClusters(&locations);
        GPUSumtimes.push_back(std::chrono::duration<double, milli>(end - start).count());
        count *= 2;
    }

    vector<double> GPUConstant;
    count = 1;
    for (int i = 0; i < amount_of_iterations; i++) {
        vector<LocationConstant> centers = GetCenterLocationsConstant(&locations_constant, indices, K / count);
        const auto start = std::chrono::steady_clock::now();
        KmeansGPUConstant(&locations_constant, 10000, K / count, &centers);
        const auto end = std::chrono::steady_clock::now();
        ResetLocationClustersConstant(&locations_constant);
        GPUConstant.push_back(std::chrono::duration<double, milli>(end - start).count());
        count *= 2;
    }
    ...
    double GPUMean = 0;
    double GPUSumMean = 0;
    double GPUConstantMean = 0;
    count = 1;
    for (int i = 0; i < amount_of_iterations; i++) {
        /*cout << "Amount of clusters: " << K / count << endl;
        cout << "CPU: " << CPUtimes[i] << "ms" << endl;
        cout << "GPU: " << GPUtimes[i] << "ms" << endl;
        cout << "GPU Constant: " << GPUConstant[i] << "ms" << endl;
        cout << "GPUS: " << GPUSumtimes[i] << "ms" << endl;*/
        cout << "Amount of clusters: " << K / count << endl;
        cout << CPUtimes[i]  << endl;
        cout << GPUtimes[i] << endl;
        cout  << GPUConstant[i]  << endl;
        cout << GPUSumtimes[i]  << endl;
        CPUMean += CPUtimes[i];
        GPUMean += GPUtimes[i];
        GPUSumMean += GPUSumtimes[i];
        GPUConstantMean += GPUConstant[i];
        count *= 2;
    }
}
int main()
{
    implementation_per_iteration();
    implementation_per_implementation();
    return 0;
}
